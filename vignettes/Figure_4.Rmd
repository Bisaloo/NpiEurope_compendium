...
dfMat %>%
  ...
  pivot_longer(!c(Country, dif.efficiency, NbStrategies, GDP, duration), names_to = "NPI") %>%
  group_by(NPI) %>%
  # Univariate model
  summarise(
#    coeff = coefficients(summary(lmer(dif.efficiency ~ NbStrategies + GDP + duration + value + (1 | Country), data = cur_data())))[5, c(1, 2)],
  ...
  mutate(diff.effect.control = est,
         ...
         .keep = "unused") %>%
  mutate(Partial = grepl("Partial$", NPI),
...
for (i in 1:24) {
  my.var <- colnames(dfMat)[i]
  # Univariate model
  model.uni <- lmer(as.formula(paste("dif.efficiency~", my.var, "+(1|Country)", sep = "")), data = dfMat)
  ...
  model.controlled <- lmer(as.formula(paste("dif.efficiency~NbStrategies+GDP+duration+", my.var, "+(1|Country)", sep = "")), data = dfMat)
...
my.table_ts$NPI <- colnames(dfMat)[1:24]
