...
#-------------------------------------------------
rm(list=ls())
library(ggplot2) ; library(hrbrthemes) ; library(corrplot) ; library(reshape); library(lme4); library(dplyr)
library(gridExtra)
...
for (i in unique(dfMat_ts$country)){
  ...
  for (j in 1:nrow(my.db)){
    pos=pos+1
    if(j==1){dfMat_ts$dif.efficiency[pos]=dfMat_ts$Efficiency[pos]}
    else{
      dfMat_ts$dif.efficiency[pos]=dfMat_ts$Efficiency[pos]-dfMat_ts$Efficiency[pos-1]
      for (k in 1:24){
        if(dfMat[pos,k]==1 & dfMat[pos-1,k]==1){dfMat_ts[pos,k]=0}
        if(dfMat[pos,k]==0 & dfMat[pos-1,k]==1){dfMat_ts[pos,k]=-1}
      }
    }
  }
}
...
npi.group.db=as.data.frame(array(NA, c(nrow(dfMat),5))) ; colnames(npi.group.db)=c('StayHome', 'MassGather', 'Teleworking', 'ClosingSchools', 'Masks')
for (j in unique(npi.group)[1:5]){
  jj=which(npi.group==j)
  if(length(jj)>1){
    npi.group.db[,j]=apply(dfMat[,jj],1,function(x){ifelse(any(x==1),1,0)})}
  else{
...
changes.plot_ts_grouped= ggplot(my.table_ts,aes(x=diff.effect.control,y=NPI.axis)) +
  ...
  theme(plot.margin = margin(0.2, 3, 0.2, 0, "cm"), axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold"),
