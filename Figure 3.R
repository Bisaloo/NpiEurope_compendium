...
#-------------------------------------------------
library(ggplot2) ; library(hrbrthemes) ; library(corrplot) ; library(reshape) ; library(grid) ; library(gridExtra)
...
strategies=lapply(strategies, `[[`, 1) ; my.table_long$NbStrategies=as.numeric(unlist(strategies))

num.pos=gregexpr('[0-9]+',my.table.se_long$NbStrategies)
strategies=regmatches(my.table.se_long$NbStrategies,num.pos)
strategies=lapply(strategies, `[[`, 1) ; my.table.se_long$NbStrategies=as.numeric(unlist(strategies))
...
nb.strategies.plot=ggplot(data=subset(my.table_long, NbStrategies>3), mapping= aes(x=NbStrategies, y=NPI, fill=efficiency))+
  geom_tile() + geom_text(aes(label = round(efficiency,2)),size=2.5) +
...
max.efficiency.plot=ggplot(tempDf, aes(X1, X2)) +
  ...
  labs(x="NPIs implemented",y="")+
  scale_x_continuous(breaks=seq(4,11,1))+
  theme(plot.margin = margin(0.2, 5, 0.2, 0, "cm"), legend.position = "none",axis.text.y=element_blank(),
